...
export type FormInstance<_TFieldValues extends FieldValues = FieldValues> = {
...
>(form: UseFormReturn<TFieldValues>): FormInstance<TFieldValues> {
  return {
    ...
    isFieldTouched: (name) => {
      const path = Array.isArray(name) ? name.join('.') : name;
      return (form.formState.touchedFields as any)[path as Path<TFieldValues>] ?? false;
    },
    isFieldsTouched: (fields) => {
      if (fields) {
        return fields.some(
          (field) => (form.formState.touchedFields as any)[field as Path<TFieldValues>],
        );
      }
